...
export default function App() {
  ...
  const unpaidSessionsCount = useMemo(
    () => teachers.reduce((s, t) => s + (t.packPaid ? 0 : (t.packCount || 0)), 0),
    [teachers]
  );
    ...
    if (wasAttended !== willAttend) {
      setTeachers((prev) =>
        prev.map((t) => {
          ...
          const prevCount = t.packCount || 0;
          const packSize = packSizeFor(t.rateType);
          if (willAttend) {
            const startsNewCycle = prevCount >= packSize;
            const count = startsNewCycle ? 1 : prevCount + 1;
            // A completed, paid pack rolling into its next session starts a fresh,
            // unpaid cycle. Otherwise leave packPaid as-is (don't touch it just
            // for attending another session mid-pack).
            return { ...t, packCount: count, packPaid: startsNewCycle ? false : t.packPaid };
          }
          return { ...t, packCount: Math.max(0, prevCount - 1) };
        })
      );
    }
    setActionModal(null);
  ...
  function markPackPaid(teacherId) {
    ...
    setTeachers((prev) => prev.map((x) => (x.id === teacherId ? { ...x, packPaid: true } : x)));
    setPayConfirm(null);
        ...
        {payConfirm && teacherMap[payConfirm] && (() => {
          ...
          return (
            <ConfirmModal
              ...
              body={`Log a payment of ${formatDZD(amount)} for ${t.name}. Their next pack starts fresh once these ${packSize} sessions are done.`}
...
function TeacherCard({ teacher, onEdit, onDelete, onPay, onScheduleSession }) {
  ...
  const isDue =
    !teacher.packPaid &&
    (teacher.dueTiming === 'first' ? (teacher.packCount || 0) >= 1 : (teacher.packCount || 0) >= packSize);
...
function TeacherFormModal({ teacher, onClose, onSave }) {
  ...
  function handleSave() {
    ...
    onSave({
      ...
      packPaid: teacher?.packPaid || false,
    });
  }
